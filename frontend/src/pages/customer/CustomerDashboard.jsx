    ...
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-5 sm:px-6">
          <h2 className="text-lg font-semibold">Punch Card Progress</h2>
          <Link to="/customer/rewards" className="text-sm font-semibold text-emerald-600 hover:text-emerald-700">View rewards</Link>
        </div>
...
function SummaryCard({ label, value, icon }) {
  return <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-slate-500">{label}</p><h2 className="mt-2 text-3xl font-bold sm:text-4xl">{value}</h2></div>{icon}</div></div>;
}
