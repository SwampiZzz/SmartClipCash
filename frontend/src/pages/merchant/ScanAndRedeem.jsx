...
export default function ScanAndRedeem() {
  ...
  return <div className="space-y-8">
    <div><h1 className="text-3xl font-bold text-slate-900">Scan &amp; Redeem</h1><p className="mt-2 text-slate-500">Scan the customer’s coupon/voucher or punch-card QR code to load the exact on-chain reward.</p></div>
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <button type="button" onClick={() => setScannerOpen(true)} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 font-semibold text-white hover:bg-emerald-700"><ScanLine size={20} />Scan reward QR</button>
    ...
    {loadedAddress && <div className="space-y-7">
      ...
      <RewardSection title="Coupons/Vouchers" empty="No redeemable coupons/vouchers found at this address.">{coupons.map((coupon) => <article key={`${coupon.txid}:${coupon.vout}`} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><TicketPercent className="text-emerald-600" /><h2 className="mt-4 text-lg font-semibold">{getCouponName(coupon.category, coupon.commitment)}</h2><p className="mt-2 text-sm text-slate-500">One-time coupon/voucher &middot; {getCouponRewardSats(coupon.category)} sats reward</p><button disabled={Boolean(pending)} onClick={() => redeemSelectedCoupon(coupon)} className="mt-5 flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 font-semibold text-white disabled:opacity-60">{pending === `coupon:${coupon.txid}:${coupon.vout}` && <LoaderCircle size={16} className="animate-spin" />}Redeem coupon/voucher</button></article>)}</RewardSection>
